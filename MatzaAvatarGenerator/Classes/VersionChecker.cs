...
static class VersionChecker
{
                ...
                for (int i = 0; i <= rows; i++)
                    if (ss[i, 0] == Assembly.GetExecutingAssembly().GetName().Name)
                    {
                        ...
                        if (CompareVersions(Application.ProductVersion.ToString(), ver) < 0)
                            ...
                            MessageBox.Show("Congratulations, you have the latest version!\n\nUpdate not necessary",
                            "No update avalabile!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
    ...
    public static void ShowUpdateDialog(string ver, string url)
    {
        if (MessageBox.Show(
            ...
            "New version: " + ver,
            "Update avalabile!", MessageBoxButtons.YesNo, MessageBoxIcon.Information)
        == DialogResult.Yes)
        {
            Process.Start(url);
            Application.Exit();
    ...
    {
        MessageBox.Show("Cannot check for new version.\n\nPlease check your internet connection!",
            "Cannot check version", MessageBoxButtons.OK, MessageBoxIcon.Error);
    }
