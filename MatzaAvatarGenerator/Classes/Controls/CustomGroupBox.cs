        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (BackColor != Color.Transparent)
                ...
                if (Parent.GetType() == typeof(CustomGroupBox))
                    ...
                    bg = Color.FromArgb(23, Color.Black);

            g.FillRectangle(new SolidBrush(bg), new Rectangle(
                borderSize + 2, borderSize + 2, Width - borderSize * 2 - 4,
                Height - borderSize * 2 - 4));

            DrawingPlus.DrawBorder(g, ClientRectangle, BorderColor, borderSize);

            titleSize = new Size();
            titleSize.Width = (int)g.MeasureString(Text, Font).Width + 2 + borderSize * 2;
            titleSize.Height = (int)g.MeasureString(Text, Font).Height + 2 + borderSize * 2;
            ...
            if (ShadowColor != Color.Transparent)
                g.DrawString(Text, Font, new SolidBrush(ShadowColor),
                    new Point(titlePosition.X + borderSize + 1, titlePosition.Y + borderSize + 1));
            g.DrawString(Text, Font, fb,
                new Point(titlePosition.X + borderSize, titlePosition.Y + borderSize));
        }
    }
}
